...
\newcommand{\eabstract}{ % 英語概要
  Moter vehicles are indispensable for world people to support them lives. On the other hand, there is the fact that 1.2 million people are killed by road acidents. Therefore the studies are has been conducted on driving support devices like Advanced Driver Assistance System(ADAS) which prevent traffic accidents. Driving model can make such system more intelligent. The driving model are well studied, however there are few interaction to surrounding vehicle in driving model. Moreover, a lane changing is strongly influenced by surrounding vehicle. Hence we studied about the lane changing model. There are several successful studies on lane changing prediction, however all have following tasks: not reveal relations about surronding vehicles, start prediction after turning the steering, not predict concrete estimation time. In this study, we estimated lane changing using surronging vehicles by assuming a distribution about starting time from lane changing, then update the distribution by sequential bayes inference. As a result, updating distribution can acquire result with less estimation error, therefore, this approach is useful to estimate time of lane changing.
}
